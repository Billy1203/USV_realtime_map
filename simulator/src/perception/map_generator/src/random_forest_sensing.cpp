...
double _x_size, _y_size, _origin_x, _origin_y, _init_x, _init_y, _resolution, _sense_rate;
...
{  
   ...
   uniform_real_distribution<double> rand_x_circle = uniform_real_distribution<double>(_x_l + 2.0, _x_h - 2.0);
   uniform_real_distribution<double> rand_y_circle = uniform_real_distribution<double>(_y_l + 2.0, _y_h - 2.0);
   ...
   uniform_real_distribution<double> rand_ellipse_c = uniform_real_distribution<double>(2.5, 6.0);
   ...
   globalMap_pcd.header.frame_id = std::string("map");
}
...
bool pubGlbObs(usv_msgs::GlbObsRcv::Request &req, usv_msgs::GlbObsRcv::Response &res)
{
   pubSensedPoints();
   return true;
}
...
{        
   ...
   _all_map_pub = n.advertise<sensor_msgs::PointCloud2>("all_map", 1, true); 

   n.param("init_state_x", _init_x,       0.0);// baselink的初始化位置
   n.param("init_state_y", _init_y,       0.0);

   n.param("map/x_size",  _x_size, 50.0);
   n.param("map/y_size",  _y_size, 50.0);
   
   n.param("map/origin_x",  _origin_x, 0.0 );//生成地图的坐标中心点
   n.param("map/origin_y",  _origin_y, 0.0 );

   n.param("map/obs_num",    _obs_num,  30);
   n.param("map/circle_num", _cir_num,  30);
   n.param("map/resolution", _resolution, 0.2);// 点云分辨率
   ...
   n.param("CircleShape/lower_circle_rad", _w_c_l, 0.3);// 半径
   n.param("CircleShape/upper_circle_rad", _w_c_h, 0.8);

   n.param("sensing/rate", _sense_rate, 1.0);

   _x_l = - _x_size / 2.0 + _origin_x;
   _x_h = + _x_size / 2.0 + _origin_x;

   _y_l = - _y_size / 2.0 + _origin_y;
   _y_h = + _y_size / 2.0 + _origin_y;
